...
# noiid 0.1 Diri high 0.5 Diri low noniid-skew Pathol
...
def record_net_data_stats(y_train, net_dataidx_map):
    ...
    log.info('mean: %.2f', np.mean(data_list))
    log.info('std: %.2f', np.std(data_list))
    log.info('Data statistics: %s' % str(net_cls_counts_dict))
